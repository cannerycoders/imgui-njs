...
export class IO
{
    ...
    installPointerPolyfill()
    {
        ...
        console.assert(window.onpointerdown === undefined);
        pointerEventProperties = ["screenX", "screenY",
                 "clientX", "clientY", "ctrlKey", "shiftKey", "altKey", 
                 "metaKey", "relatedTarget", "detail", "button", "buttons", 
                 "pointerId", "pointerType", "width", "height", "pressure", 
                 "tiltX", "tiltY", "isPrimary"];
        ...
        createMouseProxyEvent = function (type, originalEvent, noBubble) 
        {
            ...
            // Pressure is 0.5 for buttons down, 0 for no buttons down 
            // (unless pressure is reported, obvs)
        ...
        if (window.ontouchstart !== undefined) 
        {
            mouseEventIsSimulated = function(event) 
            {
                ...
                while (i--) 
                {
                    ...
                    if (Math.abs( event.clientX - touch.clientX ) < threshold && 
        ...
        if (window.ontouchstart !== undefined) 
        {
            ...
            createTouchProxyEvent = function (type, originalEvent, touch, 
                                                noBubble, relatedTarget) 
            {
                var params = 
                {
                    screenX:       originalEvent.screenX,
            ...
            window.addEventListener( "touchmove", function (event) 
            {
                ...
                }; // end processTouch function
        
                for ( i=0; i<touches.length; i+=1 ) 
                {
                    processTouch( touches[i] );
                }
            }); // end touchmove handler
