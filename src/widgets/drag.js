...
export var ImguiDragMixin =
{
                ...
                power=1, onChange=null)
    {
        ...
        return this.DragScalarN(label, DataType.Float, val, 2,
                                v_speed, v_min, v_max, format, power, onChange);
    },
    ...
    DragFloatRange2(label, v_current_min, v_current_max, v_speed=1,
                v_min=0, v_max=0, format=null, format_max=null, power=1, onChange)
    {
        ...
        let value_changed = this.DragFloat("##min", v_current_min, v_speed,
            ...
            format, power, (newval)=>v_current_min = newval);
        ...
        value_changed |= this.DragFloat("##max", v_current_max, v_speed,
            ...
            format_max ? format_max : format, power, (newval)=>v_current_max=newval);
        this.PopItemWidth();
        this.SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    DragInt(label, v, v_speed=1, v_min=0, v_max=0, format=null, onChange=null)
    {
    ...
    // If v_min >= v_max we have no bound
    DragInt2(label, v, v_speed=1, v_min=0, v_max=0, format=null, onChange=null)
    {
        if(format==null) format=IFmt;
    ...
    // If v_min >= v_max we have no bound
    DragInt3(label, v, v_speed=1, v_min=0, v_max=0, format=null, onChange=null)
    {
        if(format==null) format=IFmt;
    ...
    // If v_min >= v_max we have no bound
    DragInt4(label, v, v_speed=1, v_min=0, v_max=0, format=null, onChange=null)
    {
        if(format==null) format=IFmt;
        return this.DragScalarN(label, DataType.S32, v, 4, v_speed,
                                v_min, v_max, format, 1, onChange);
    },

    // If v_min >= v_max we have no bound
    DragIntRange2(label, v_current_min, v_current_max,
                  v_speed=1, v_min=0, v_max=0,
                  format=null, format_max=null, onChange)
    {
        let win = this.getCurrentWindow();
        if (win.SkipItems)
            return false;
    ...
    DragScalarN(label, data_type, v, components, v_speed=1, v_min=0, v_max=0,
                format=null, power=1, onChange=null)
    {
        let win = this.getCurrentWindow();
        if (win.SkipItems)
            return false;
    ...
    dragBehavior(id, data_type, v, v_speed, v_min, v_max, format, power, flags,
                onChange)
    {
        let g = this.guictx;
        if(g.ActiveId == id)
